...
json_data['single_instance_variables'].forEach(function(mVariable) {
  ...
  serverString += "app.get(\'/demo/" + mVariable.name.toLowerCase() + "\', function(req, res) { redis.get(\'" + mVariable.name.toLowerCase() + "\', function(err, result) { res.json({ value: " + resultString + " }); }); });\n";
  serverString += "app.post(\'/demo/" + mVariable.name.toLowerCase() + "\', function(req, res) { redis.set(\'" + mVariable.name.toLowerCase() + "\', req.body.value, function(err, result) { res.json(true); }); });\n\n";
});

serverString += "app.listen(8080);\n";
